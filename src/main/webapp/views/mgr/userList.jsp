...
<script langeuage="javascript">
	...
	$(function () {  
	    setTimeout(function () {
	    	if($("#psw").val()){
	    		alert("用户名："+$("#un").val()+"，密码："+$("#psw").val()+",请及时通知用户修改。");
	    	}
	    }, 500);  
	})
</script>
...
<body>
	...
	<div class="content">
		...
		<form id="list_form" action="del" method="post">
					...
					<c:forEach items="${list.result }" var="item" varStatus="sn">
						<tr>
							...
							<td style="width: 130px"><console:input
									funcnum="p_login_user_edit" type="button" cssClass="btn btn-info"
									value="修改" onclick="editItem('${item.id}')"></console:input>
								&nbsp; <console:input funcnum="p_login_user" type="button"
			...
			<console:input funcnum="p_login_user_add" cssClass="btn btn-primary"
				style="float: left;" type="button" onclick="goAdd()" value="增加"></console:input>
			&nbsp;&nbsp;
			<console:input funcnum="p_login_user" cssClass="btn btn-warning"
